...
{
    if (thread_number <= 0 || max_requests <= 0)  //线程池中线程的数量<=0 最大请求数<=0
    ...
    {
        if (pthread_create(m_threads + i, NULL, worker, this) != 0)      //创建线程池
        {
            delete[] m_threads;
            throw std::exception();
        }
...
bool threadpool<T>::append(T *request, int state)
{
    ...
    if (m_workqueue.size() >= m_max_requests)    //如果超出最大请求数
    {
        m_queuelocker.unlock();
        return false;
    }
    request->m_state = state;             //读/写
...
template <typename T>
bool threadpool<T>::append_p(T *request)    //无状态
{
    m_queuelocker.lock();
...
void threadpool<T>::run()
{
    while (true)
    {
        ...
        T *request = m_workqueue.front();    //获得一个请求
        m_workqueue.pop_front();
        m_queuelocker.unlock();              //关锁
        if (!request)
            continue;
